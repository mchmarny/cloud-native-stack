...
func NewResult(bundlerType BundleType) *Result {
	return &Result{
		Type:    bundlerType,
		Files:   make([]string, 0),
...
func (o *Output) ByType() map[BundleType]*Result {
	results := make(map[BundleType]*Result, len(o.Results))
	for _, r := range o.Results {
		results[r.Type] = r
	}
	return results
...
func (o *Output) FailedBundlers() []BundleType {
	failed := make([]BundleType, 0, len(o.Errors))
...
func (o *Output) SuccessfulBundlers() []BundleType {
	successful := make([]BundleType, 0, len(o.Results))
	for _, r := range o.Results {
		if r.Success {
